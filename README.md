...
3. Click Generate Demand Letter (optionally add custom AI instructions). OpenAI produces structured JSON sections: Introduction, Liability, Damages, Demand, Closing.
...
### Env Requirements
Ensure `VITE_SUPABASE_FUNCTIONS_URL` and `OPENAI_API_KEY` are set. Vectorization also requires OpenAI + Pinecone keys configured on the Supabase Edge Function.
